...
async function testForwardTo(testReg, identityManager, proxyAddress, fromAccount, shouldEqual) {
  ...
  try {
    await identityManager.forwardTo(proxyAddress, testReg.address, 0, '0x' + data, {
      from: fromAccount
    })
  } catch (error) {
...
contract('IdentityManager', (accounts) => {
  ...
  describe('existing identity', () => {
    ...
    it('owner can add other owner', async function () {
      ...
      let tx = await identityManager.addOwner(proxy.address, user5, {
        from: user1
      })
      let log = tx.logs[0]
    ...
    it('owner can not add owner that is already an owner', async function () {
      await identityManager.addOwner(proxy.address, user2, {
        from: user1
      })
      await evm_increaseTime(adminTimeLock + 1)
      let errorThrown = false
      try {
        let tx = await identityManager.addOwner(proxy.address, user2, {
          from: user1
        })
      } catch (e) {
        assert.match(e.message, /revert/, "should have thrown")
    ...
    it('owner is rateLimited on some functions', async function () {
      ...
      let errorThrown = false
      try {
        await identityManager.addOwner(proxy.address, user4, {
          from: user1
        })
      } catch (e) {
        assert.match(e.message, /revert/, "should have thrown")
      ...
      errorThrown = false
      try {
        await identityManager.removeOwner(proxy.address, user5, {
          from: user1
        })
      } catch (e) {
        assert.match(e.message, /revert/, "should have thrown")
      ...
      errorThrown = false
      try {
        await identityManager.changeRecovery(proxy.address, recoveryKey2, {
          from: user1
        })
      } catch (e) {
        assert.match(e.message, /revert/, "should have thrown")
      ...
      tx = await identityManager.addOwner(proxy.address, user4, {
        from: user1
      })
      log = tx.logs[0]
      assert.equal(log.event, 'LogOwnerAdded', 'should trigger correct event') //tests for correctness elsewhere
      ...
      //no longer rateLimited
      await evm_increaseTime(adminTimeLock + 1)
      tx = await identityManager.removeOwner(proxy.address, user5, {
        from: user1
      })
      log = tx.logs[0]
      ...
      //no longer rateLimited
      await evm_increaseTime(adminTimeLock + 1)
      tx = await identityManager.changeRecovery(proxy.address, recoveryKey2, {
        from: user1
      })
      log = tx.logs[0]
    ...
    describe('new owner added by owner', () => {
      ...
      it('can not remove other owner yet', async function () {
        try {
          await identityManager.removeOwner(proxy.address, user1, {
            from: user2
          })
        } catch (error) {
          assert.match(error.message, /VM Exception while processing transaction: revert/, 'throws an error')
      ...
      describe('after adminTimeLock', () => {
        ...
        it('is olderOwner', async function () {
          ...
          let tx = await identityManager.removeOwner(proxy.address, user1, {
            from: user2
          })
        ...
        it('can add new owner', async function () {
          let tx = await identityManager.addOwner(proxy.address, user3, {
            from: user2
          })
          const log = tx.logs[0]
          assert.equal(log.args.owner,
        ...
        it('can not remove yourself as owner', async function () {
          let errorThrown = false
          try {
            await identityManager.removeOwner(proxy.address, user2, {
              from: user2
            })
          } catch (e) {
            assert.match(e.message, /revert/, "should have thrown")
        ...
        it('can change recoveryKey', async function () {
          ...
          let tx = await identityManager.changeRecovery(proxy.address, recoveryKey2, {
            from: user2
          })
          const log = tx.logs[0]
          assert.equal(log.args.recoveryKey,
        ...
        it('should throw if recoveryKey is set to zero address', async function () {
          let errorThrown = false
          try {
            await identityManager.changeRecovery(proxy.address, ZERO_ADDRESS, {
              from: user1
            })
          } catch (e) {
            assert.match(e.message, /revert/, "should have thrown")
    ...
    describe('new owner added by recoveryKey', () => {
      beforeEach(async function () {
        await identityManager.addOwnerFromRecovery(proxy.address, user2, {
          from: recoveryKey
        })
      })
      ...
      it('incorrect recoveryKey should throw', async function () {
        let errorThrown = false
        try {
          await identityManager.addOwnerFromRecovery(proxy.address, user4, {
            from: nobody
          })
        } catch (e) {
          assert.match(e.message, /revert/, "should have thrown")
  ...
  describe('migration', () => {
    ...
    it('older owner can start transfer', async function () {
      let tx = await identityManager.initiateMigration(proxy.address, newIdenManager.address, {
        from: user1
      })
      let log = tx.logs[0]
    ...
    it('young owner should not be able to start transfer', async function () {
      ...
      try {
        await identityManager.initiateMigration(proxy.address, newIdenManager.address, {
          from: user2
        })
      } catch (error) {
        assert.match(error.message, /VM Exception while processing transaction: revert/, 'throws an error')
    ...
    it('correct keys can cancel migration', async function () {
      ...
      tx = await identityManager.cancelMigration(proxy.address, {
        from: user1
      })
      log = tx.logs[0]
      ...
      tx = await identityManager.initiateMigration(proxy.address, newIdenManager.address, {
        from: user1
      })
      ...
      let threwError = false
      try {
        await identityManager.cancelMigration(proxy.address, {
          from: nobody
        })
      } catch (error) {
        assert.match(error.message, /VM Exception while processing transaction: revert/, 'throws an error')
    ...
    it('correct keys should finilize transfer', async function () {
      ...
      let threwError = false
      try {
        await identityManager.finalizeMigration(proxy.address, {
          from: nobody
        })
      } catch (error) {
        assert.match(error.message, /VM Exception while processing transaction: revert/, 'throws an error')
      ...
      let tx = await identityManager.finalizeMigration(proxy.address, {
        from: user1
      })
      let log = tx.logs[0]
    ...
    it('should be owner of new identityManager after successful transfer', async function () {
      ...
      await identityManager.forwardTo(proxy.address, newIdenManager.address, 0, data, {
        from: user1
      })
      ...
      await newIdenManager.forwardTo(proxy.address, testReg.address, 0, data, {
        from: user1
      })
    ...
    it('should throw if trying to register an existing proxy', async function () {
      ...
      let data = '0x' + lightwallet.txutils._encodeFunctionTxData('registerIdentity', ['address', 'address'], [user1, recoveryKey])
      try {
        await identityManager.forwardTo(proxy.address, identityManager.address, 0, data, {
          from: user1
        })
      } catch (e) {
