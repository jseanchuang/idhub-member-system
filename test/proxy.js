...
async function testProxyTx(testReg, proxy, fromAccount, shouldEqual) {
  ...
  try {
    await proxy.forward(testReg.address, 0, '0x' + data, {
      from: fromAccount
    })
  } catch (e) {
    errorThrown = true
...
contract('Proxy', (accounts) => {
  ...
  before(async function () {
    ...
    proxy1 = await Proxy.new({
      from: ownerProxy1
    })
    proxy2 = await Proxy.new({
      from: ownerProxy2
    })
  ...
  it('Event emitted on tx from proxy', async function () {
    ...
    // Send forward request from the owner
    let tx = await proxy1.forward(testReg.address, 0, data, {
      from: accounts[0]
    })
    let log = tx.logs[0]
  ...
  it('Should send ether correctly', async function () {
    ...
    let tx = await proxy1.forward(receiver, web3.toWei(ethToSend, 'ether'), '0x0', {
      from: ownerProxy1
    })
    let log = tx.logs[0]
  ...
  it('Should send small amount of ether correctly', async function () {
    ...
    let tx = await proxy1.forward(receiver, weiToSend, '0x0', {
      from: ownerProxy1
    })
    let log = tx.logs[0]
  ...
  it('Should throw if sending too much ether', async function () {
    ...
    try {
      await proxy1.forward(receiver, web3.toWei(ethToSend, 'ether'), '0x0', {
        from: ownerProxy1
      })
    } catch (e) {
      errorThrown = true
  ...
  it('Should be able to empty out ether from proxy', async function () {
    ...
    let balance = await web3.eth.getBalanceAsync(proxy1.address)
    let tx = await proxy1.forward(receiver, balance, '0x0', {
      from: ownerProxy1
    })
    balance = await web3.eth.getBalanceAsync(proxy1.address)
  ...
  it('Should throw if function call fails', async function () {
    ...
    try {
      await proxy1.forward(testReg.address, 0, '0x' + data, {
        from: accounts[0]
      })
    } catch (e) {
      errorThrown = true
