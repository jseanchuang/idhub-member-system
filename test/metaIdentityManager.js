...
async function testForwardTo(testReg, identityManager, proxyAddress, fromAccount, claimedFrom, shouldEqual) {
  ...
  try {
    await identityManager.forwardTo(claimedFrom, proxyAddress, testReg.address, 0, '0x' + data, {
      from: fromAccount
    })
  } catch (e) {
...
async function testForwardToFromRelay(testReg, identityManager, proxyAddress, fromAccount, txRelayAddress, shouldEqual) {
  ...
  // Send forward request from the owner
  try {
    await identityManager.forwardTo(fromAccount, proxyAddress, testReg.address, 0, '0x' + data, {
      from: txRelayAddress
    })
  } catch (e) {
    //errorThrown = e.message
...
contract('MetaIdentityManager', (accounts) => {
  ...
  it('Correctly creates Identity and calls registry set', async function () {
    let testNum = getRandomNumber()
    let data = lightwallet.txutils._encodeFunctionTxData('register', ['uint256'], [testNum])
    let tx = await identityManager.createIdentityWithCall(user1, recoveryKey, testReg.address, '0x' + data, {
      from: nobody
    })
    let log = tx.logs[0]
    assert.equal(log.event, 'LogIdentityCreated', 'wrong event')
  ...
  describe('existing identity', () => {
    ...
    it('onlyAuthorized modifier does not allow transactions when data for metatx is not properly formed starting with address', async function () {
      let errorThrown = false
      try {
        await identityManager.forwardTo('0x0', '0x0', '0x0', 0, '0x0', {
          from: relay
        })
      } catch (e) {
        assert.match(e.message, /revert/, "Should have thrown")
    ...
    it('owner can add other owner', async function () {
      let isOwner = await identityManager.isOwner(proxy.address, user5, {
        from: user1
      })
      assert.isFalse(isOwner, 'user5 should not be owner yet')
      let tx = await identityManager.addOwner(user1, proxy.address, user5, {
        from: user1
      })
      let log = tx.logs[0]
      ...
      isOwner = await identityManager.isOwner(proxy.address, user5, {
        from: user1
      })
    ...
    it('owner can not add owner that is already an owner', async function () {
      await identityManager.addOwner(user1, proxy.address, user2, {
        from: user1
      })
      await evm_increaseTime(adminTimeLock + 1)
      try {
        let tx = await identityManager.addOwner(user1, proxy.address, user2, {
          from: user1
        })
      } catch (e) {
        assert.match(e.message, /revert/, "should have thrown")
    ...
    it('owner is rateLimited on some functions', async function () {
      ...
      let errorThrown = false
      try {
        await identityManager.addOwner(user1, proxy.address, user4, {
          from: user1
        })
      } catch (e) {
        //assert.match(e.message, /revert/, "should have thrown")
      ...
      errorThrown = false
      try {
        await identityManager.removeOwner(user1, proxy.address, user5, {
          from: user1
        })
      } catch (e) {
        //assert.match(e.message, /revert/, "should have thrown")
      ...
      errorThrown = false
      try {
        await identityManager.changeRecovery(user1, proxy.address, recoveryKey2, {
          from: user1
        })
      } catch (e) {
        //assert.match(e.message, /revert/, "should have thrown")
      ...
      tx = await identityManager.addOwner(user1, proxy.address, user4, {
        from: user1
      })
      log = tx.logs[0]
      assert.equal(log.event, 'LogOwnerAdded', 'should trigger correct event') //tests for correctness elsewhere
      ...
      //no longer rateLimited
      await evm_increaseTime(adminTimeLock + 1)
      tx = await identityManager.removeOwner(user1, proxy.address, user5, {
        from: user1
      })
      log = tx.logs[0]
      ...
      //no longer rateLimited
      await evm_increaseTime(adminTimeLock + 1)
      tx = await identityManager.changeRecovery(user1, proxy.address, recoveryKey2, {
        from: user1
      })
      log = tx.logs[0]
    ...
    it('non-owner can not add other owner', async function () {
      errorThrown = false
      try {
        await identityManager.addOwner(user3, proxy.address, user4, {
          from: user3
        })
      } catch (e) {
    ...
    describe('new owner added by owner', () => {
      ...
      it('can not add other owner yet', async function () {
        let errorThrown = false
        try {
          await identityManager.addOwner(user2, proxy.address, user4, {
            from: user2
          })
        } catch (e) {
          //assert.match(e.message, /revert/, 'throws an error')
      ...
      it('can not remove other owner yet', async function () {
        let errorThrown = false
        try {
          await identityManager.removeOwner(user2, proxy.address, user1, {
            from: user2
          })
        } catch (e) {
          //assert.match(e.message, /revert/, 'throws an error')
      ...
      it('can not change recoveryKey yet', async function () {
        let errorThrown = false
        try {
          await identityManager.changeRecovery(user2, proxy.address, recoveryKey2, {
            from: user2
          })
        } catch (e) {
          //assert.match(e.message, /revert/, 'throws an error')
      ...
      describe('after adminTimeLock', () => {
        ...
        it('is olderOwner', async function () {
          ...
          let tx = await identityManager.removeOwner(user2, proxy.address, user1, {
            from: user2
          })
        ...
        it('can add new owner', async function () {
          let tx = await identityManager.addOwner(user2, proxy.address, user3, {
            from: user2
          })
          const log = tx.logs[0]
        ...
        it('can not remove yourself as owner', async function () {
          let errorThrown = false
          try {
            await identityManager.removeOwner(user2, proxy.address, user2, {
              from: user2
            })
          } catch (e) {
            assert.match(e.message, /revert/, "should have thrown")
        ...
        it('can change recoveryKey', async function () {
          let isRecovery = await identityManager.isRecovery(proxy.address, recoveryKey2, {
            from: user1
          })
          assert.isFalse(isRecovery, 'recoveryKey2 should not be recovery yet')
          let tx = await identityManager.changeRecovery(user2, proxy.address, recoveryKey2, {
            from: user2
          })
          const log = tx.logs[0]
          assert.equal(log.args.recoveryKey,
          ...
          assert.equal(log.args.instigator,
            user2,
            'Instigator key is set in event')
          isRecovery = await identityManager.isRecovery(proxy.address, recoveryKey2, {
            from: user1
          })
        ...
        it('should throw if recoveryKey is set to zero address', async function () {
          let errorThrown = false
          try {
            await identityManager.changeRecovery(user2, proxy.address, ZERO_ADDRESS, {
              from: user2
            })
          } catch (e) {
            //assert.match(e.message, /revert/, "should have thrown")
    ...
    describe('new owner added by recoveryKey', () => {
      beforeEach(async function () {
        await identityManager.addOwnerFromRecovery(recoveryKey, proxy.address, user2, {
          from: recoveryKey
        })
      })
      ...
      it('recoveryKey is rate limited in added new owners', async function () {
        ...
        let errorThrown = false
        try {
          tx = await identityManager.addOwnerFromRecovery(recoveryKey, proxy.address, user4, {
            from: recoveryKey
          })
        } catch (e) {
          //assert.match(e.message, /revert/, "should have thrown")
      ...
      it('incorrect recoveryKey should throw', async function () {
        let errorThrown = false
        try {
          await identityManager.addOwnerFromRecovery(nobody, proxy.address, user4, {
            from: nobody
          })
        } catch (e) {
          //assert.match(e.message, /revert/, "should have thrown")
  ...
  describe('migration', () => {
    ...
    beforeEach(async function () {
      ...
      tx = await identityManager.addOwner(user1, proxy.address, user2, {
        from: user1
      })
      log = tx.logs[0]
    ...
    it('older owner can start transfer', async function () {
      let tx = await identityManager.initiateMigration(user1, proxy.address, newIdenManager.address, {
        from: user1
      })
      let log = tx.logs[0]
    ...
    it('young owner should not be able to start transfer', async function () {
      ...
      try {
        await identityManager.initiateMigration(user2, proxy.address, newIdenManager.address, {
          from: user2
        })
      } catch (e) {
        //assert.match(e.message, /revert/, 'throws an error')
    ...
    it('non-owner should not be able to start transfer', async function () {
      let threwError = false
      try {
        await identityManager.initiateMigration(nobody, proxy.address, newIdenManager.address, {
          from: nobody
        })
      } catch (e) {
        //assert.match(e.message, /revert/, 'throws an error')
    ...
    it('correct keys can cancel migration', async function () {
      ...
      tx = await identityManager.cancelMigration(user1, proxy.address, {
        from: user1
      })
      log = tx.logs[0]
      ...
      await evm_increaseTime(userTimeLock)
      tx = await identityManager.cancelMigration(user2, proxy.address, {
        from: user2
      })
      ...
      await identityManager.initiateMigration(user1, proxy.address, newIdenManager.address, {
        from: user1
      })
      //Don't need to check setup again
      let threwError = false
      try {
        await identityManager.cancelMigration(nobody, proxy.address, {
          from: nobody
        })
      } catch (e) {
        //assert.match(e.message, /revert/, 'throws an error')
    ...
    it('correct keys should finilize transfer', async function () {
      ...
      let threwError = false
      try {
        await identityManager.finalizeMigration(nobody, proxy.address, {
          from: nobody
        })
      } catch (e) {
        //assert.match(e.message, /revert/, 'throws an error')
      ...
      threwError = false
      try {
        await identityManager.finalizeMigration(user2, proxy.address, {
          from: user2
        })
      } catch (e) {
        //assert.match(e.message, /revert/, 'throws an error')
      ...
      threwError = false
      try {
        await identityManager.finalizeMigration(user1, proxy.address, {
          from: user1
        })
      } catch (e) {
        //assert.match(e.message, /revert/, 'throws an error')
      ...
      let tx = await identityManager.finalizeMigration(user1, proxy.address, {
        from: user1
      })
      let log = tx.logs[0]
      ...
      let isOwner = await identityManager.isOwner(proxy.address, user1, {
        from: user1
      })
      assert.isFalse(isOwner, 'user1 should not be owner anymore')

      let isRecovery = await identityManager.isRecovery(proxy.address, recoveryKey, {
        from: user1
      })
    ...
    it('should be owner of new identityManager after successful transfer', async function () {
      ...
      await identityManager.forwardTo(user1, proxy.address, newIdenManager.address, 0, data, {
        from: user1
      })
      ...
      await newIdenManager.forwardTo(user1, proxy.address, testReg.address, 0, data, {
        from: user1
      })
    ...
    it('should throw if trying to register an existing proxy', async function () {
      ...
      let data = '0x' + lightwallet.txutils._encodeFunctionTxData('registerIdentity', ['address', 'address'], [user1, recoveryKey])
      try {
        await identityManager.forwardTo(user1, proxy.address, identityManager.address, 0, data, {
          from: user1
        })
      } catch (e) {
        //assert.match(e.message, /revert/, 'throws an error')
