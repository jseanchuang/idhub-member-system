...
contract('TxRelay', (accounts) => {
  ...
  describe("Whitelist", () => {
    ...
    it("Should register a list of addresses to a whitelist", async () => {
      await testTxRelay.addToWhitelist([sender2, sender3], {
        from: sender1
      })
    ...
    it("Should allow owner of list to add another sender", async () => {
      await testTxRelay.addToWhitelist([sender4], {
        from: sender1
      })
    ...
    it("Should not allow non-owner to add another sender", async () => {
      await testTxRelay.addToWhitelist([sender5], {
        from: sender2
      })
    ...
    it("Should allow owner of list to remove senders", async () => {
      await testTxRelay.removeFromWhitelist([sender4, sender3], {
        from: sender1
      })
    ...
    it("Removing sender not on list should not have an effect", async () => {
      ...
      await testTxRelay.removeFromWhitelist([sender3], {
        from: sender1
      })
    ...
    it("Should allow another sender to create their own whitelist", async () => {
      await testTxRelay.addToWhitelist([sender6, sender7], {
        from: sender2
      })
    ...
    it("Should send tx if sender is in whitelist", async () => {
      ...
      await testTxRelay.relayMetaTx(p.v, p.r, p.s, p.dest, p.data, sender1, {
        from: sender2
      })
    ...
    it("Should throw if sender is in incorrect whitelist", async () => {
      ...
      errorThrown = false
      try {
        await testTxRelay.relayMetaTx(p.v, p.r, p.s, p.dest, p.data, sender2, {
          from: sender2
        })
      } catch (e) {
        assert.match(e.message, /revert/, "Should have thrown")
